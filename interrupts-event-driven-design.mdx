...
<PodcastEmbed src="https://open.spotify.com/episode/6TNUlZqNPsXlYCVHUrP6Ng?si=jMPyQDM_Q9SaDHcOUcBbtg" />
...
![Arduino Nano with push button on PD2 for external interrupt reaction timer](https://cdn.siliconwit.com/education/embedded-programming-atmega328p/Nano-LED-Button-Interrupt.png)
...
```mermaid
flowchart TD
    A["Main code running"] --> B["Event occurs\ne.g. INT0 falling edge"]
    B --> C["CPU finishes current instruction"]
    C --> D["Push PC onto stack,\ndisable global IRQ"]
    D --> E["Jump to vector address\ne.g. 0x0002"]
    E --> F["Execute ISR\nyour handler code"]
...
sequenceDiagram
    ...
    Note over M: counter = 0x00FF
    M->>M: Read low byte (gets 0xFF)
    I-->>I: ISR fires!
    Note over I: counter changes<br/>0x00FF to 0x0100
    I-->>M: ISR returns
    M->>M: Read high byte (gets 0x01)
    Note over M: Result: 0x01FF (corrupt!)<br/>ATOMIC_BLOCK prevents this
```
