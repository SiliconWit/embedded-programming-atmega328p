...
description: "Master ATmega328P low-power techniques including all six sleep modes, watchdog timer configuration, BOD disable, and EEPROM storage. Build a battery-powered door open alert that sleeps in power-down mode, wakes on a magnetic reed switch, and counts events in non-volatile memory."
contributors: "sam-macharia,lawrence-langat,joseph-disamalu"
pubDate: 2026-03-12
tags: ["power-management", "sleep-modes", "watchdog", "battery", "low-power", "eeprom", "atmega328p", "avr"]
...
```mermaid
flowchart TD
    A["Idle\n~3 mA\nAny interrupt wakes"] --> B["ADC Noise Reduction\n~1 mA\nADC, ext int, Timer2"]
    B --> C["Power-save\n~1 uA\nTimer2, ext int"]
    C --> D["Power-down\n~0.1 uA\nINT0/1, PCINT, TWI"]
    D --> E["Standby\n~0.2 uA\nSame + faster wake"]
    E --> F["Ext. Standby\n~0.2 uA\n+ Timer2 async"]
...
```mermaid
flowchart TD
    A["Power on"] --> B["Init GPIO,\nWDT, INT0"]
    B --> C["Enter Power-down\nsleep"]
    C --> D{"Wake source?"}
    D -->|WDT| E["Blink LED once"]
    E --> C
    D -->|"INT0 (door)"| F["Beep 2s,\nBlink 3x"]
    F --> C
```
...
| Register | Purpose |
|----------|---------|
| EEARH:EEARL | Address (0 to 1023) |
| EEDR | Data byte to read or write |
| EECR | Control: EEMPE (master program enable), EEPE (program enable), EERE (read enable) |
...
:::tip[Instrument it]
A field sensor has to do two things you never see: last on its battery and recover on its own. Sleep handles the first, dropping the ATmega to microamps between readings so a coin cell lasts; the watchdog handles the second, resetting a node that has hung so it does not silently stop reporting halfway through the season. A monitor you can forget about is one that sleeps deep and restarts itself, then keeps [sending data](/education/iot-systems/) to [siliconwit.io](https://siliconwit.io) without a site visit.
:::
