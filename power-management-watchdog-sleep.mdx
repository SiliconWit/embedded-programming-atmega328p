...
The ATmega328P has six sleep modes, each disabling progressively more hardware to save power. The deeper the sleep, the fewer wake sources are available and the longer it takes to resume execution. Power-down mode disables everything except the external interrupt logic and the watchdog timer, achieving the lowest possible current draw.
