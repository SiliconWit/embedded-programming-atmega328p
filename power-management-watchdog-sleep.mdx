...
:::tip[Instrument it]
A field sensor has to do two things you never see: last on its battery and recover on its own. Sleep handles the first, dropping the ATmega to microamps between readings so a coin cell lasts; the watchdog handles the second, resetting a node that has hung so it does not silently stop reporting halfway through the season. A monitor you can forget about is one that sleeps deep and restarts itself, then keeps [sending data](/education/iot-systems/) without a site visit.
:::
