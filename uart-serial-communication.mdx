...
A ring buffer (circular buffer) decouples the producer (main code adding characters) from the consumer (UART hardware sending them). The main code writes to the buffer and enables the UDRE interrupt. The ISR sends one byte at a time and disables itself when the buffer is empty. This prevents the main loop from stalling while long strings are transmitted.
